...
class ReportController extends Controller
{
    ...
    public function AllTax(Request $request){
        ...
        if (!auth()->user()->can('all_tax_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
          ...
          $expenses = $this->transactionUtil->getexpensesTotals($business_id, $start_date, $end_date, $location_id);;
       

       $purchases = $this->transactionUtil->getPurchaseTotals($business_id, $start_date, $end_date, $location_id);

       $sells = $this->transactionUtil->getSellTotals(
           $business_id,
           $start_date,
           $end_date,
           $location_id
       ...
       $returns = $this->transactionUtil->getReturnsTotals($business_id, $start_date, $end_date, $location_id);
    ...
    $difference = $purchases['total_purchase_return_inc_tax'] -$sells['total_sell_inc_tax'] - $returns['total_purchase_return_inc_tax']- $expenses['total_purchase_return_inc_tax
        '];
            ...
            return [
                ...
                'difference'=>$difference,
                   ...
                   $business_locations = BusinessLocation::forDropdown($business_id, true);
// return $purchases[0]['amount_paid'] -$sells[0]['amount_paid'] - $expenses[0]['amount_paid'] -$purchases_returns[0]['amount_paid'] ;

        return view('report.tax_due')->with(compact('business_locations'));
    }
